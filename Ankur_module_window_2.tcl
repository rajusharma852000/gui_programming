...
# to get desired id by seaching in ID combobox
proc filter_combobox {event} {
    ...
    .top.id_combo configure -values $filtered_list
     ...
     #Manually select the text again to allow further typing
    .top.id_combo icursor end
...
proc add_comment { new_comment} {
    global selected_position main_file has_unsaved_changes selected_pane  ;# Access global variables
    set default_pane 1;
    ...
    set match_list [regexp -inline -expanded $pane_regex $fileContent]
    # puts "match list: $match_list\n"
    # If the selected pane doesn't exist, fallback to pane 1
    if {[llength $match_list] == 0 && $selected_pane != $default_pane} {
         puts "Pane $selected_pane not found! Falling back to Pane $default_pane."
        #puts "Hi! \n"
        set pane_regex "\\\\setpanenumber\\{$default_pane\\}\\s*(.*?)(?=\\\\setpanenumber\\{|\\\\end\\{document\\}}|\\\\newpage)"
        set match_list [regexp -inline -expanded $pane_regex $fileContent]
         # puts "Fallback regex capturing: $match_list"
     }
    ...
    if {[llength $match_list] > 0} {
       ...
       #  puts "Matched pane content: $matched_pane"
    } else {
    ...
    if {[regexp $comment_regex $matched_pane match]} {
        set modifiedContent [regsub $comment_regex $matched_pane "\\putcomment$selected_position\{$new_comment\}"]
        # puts "modified Content: $modifiedContent\n"
    } else {
    ...
    puts -nonewline $fileId $modifiedFileContent
    close $fileId
...
proc add_marks { args } {
    global main_file has_unsaved_changes num_checkboxes selected_pane ;# Access global variables
    set has_unsaved_changes "true" ;# update the variable
    set default_pane 1;
    ...
    if {[regexp $marks_regex $matched_pane match]} {
        set modifiedContent [regsub $marks_regex $matched_pane "\\putmarks\{$new_marks\}"]
    } else {
...
proc update_comment {args} {
    global selected_pane selected_position  curr_dir main_file current_file;# Access global variables
    ...
    #puts "update_comment called with selected_pane: $selected_pane"
    ## Improved regex to match the specific pane section
    set pane_regex "\\\\setpanenumber\\{$selected_pane\\}\\s*(.*?)(?=\\\\setpanenumber\\{|\\\\end\\    {document\\}}|\\\\newpage)"
    ...
    # Debug print to see what the regex is capturing
    # puts "Regex capturing: $match_list"
...
proc update_marks { } {
    global curr_dir main_file selected_pane
    set default_pane 1
    ...
    set putmarks ""
    if {[regexp $putmarks_regex $matched_pane -> put_match]} {
...
# Generate pane numbers from 1 to max_panes
set pane_values [generate_pane_list $max_panes]
# puts "no. of panes it has: $pane_values\n";
...
ttk::combobox .top.id_combo -values $tex_ids -font myFont -postcommand {filter_combobox %A}
.top.id_combo set [lindex $tex_ids 0] ;# Set first file name as default value
 bind .top.id_combo <KeyRelease> {filter_combobox %A} 
...
ttk::combobox .top.panel_combo -values $pane_values -textvariable selected_pane -font myFont
...
# this trigger when you change the pane
# Wrapper function to call multiple functions when selected_pane changes
...
trace add variable selected_pane write on_selected_pane_change
