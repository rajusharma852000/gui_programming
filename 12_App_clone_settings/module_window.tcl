...
proc read_settings { key } {
    set settings_path "$::current_folder/.Settings.config"

    if {[file exists $settings_path]} {
        set file_id [open $settings_path r]
...
proc create_time_spent_file { } {
    ...
    # Correctly create build directory inside $curr_dir
...
proc run_pdflatex { main_tex } {
   ...
   # Correctly create build directory inside $curr_dir
    ...
    # read settigs
    set latex_compile_command [read_settings "latex_compile_command"];
    if { $latex_compile_command eq "not found" } {
        set latex_compile_command "pdflatex -file-line-error -interaction=nonstopmode -output-directory=build"
    }
    
    
    
    
     # Try to run the LaTeX compile command
if { [catch {exec {*}$latex_compile_command $main_tex > /dev/null &} result] } {
...
proc update_comment_timer {} {
    ...
    # read from settings
    set user_pdf_timer_interval [read_settings "const_generate_pdf_timer_interval_msecs"];
    if { $user_pdf_timer_interval eq "not found" } {
        set user_pdf_timer_interval 750;
    }
    
    
    after cancel $::comment_timer_id  ; # Cancel any existing timer
    set ::comment_timer_id [after $user_pdf_timer_interval [list add_comment $new_comment]]  ; # Set a new timer for 0.750 seconds
}
