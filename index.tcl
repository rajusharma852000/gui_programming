...
button .top.select_project -text "Select Project" -command select_folder -font myFont -underline 7
...
label .bottom.module_label -text "Select Module" -font myFont -state disabled -underline 7
ttk::combobox .bottom.module_combo -width 25 -state readonly -state disabled -font largeFont
button .bottom.start_button -text "Start/Resume" -state disabled -command open_module_window -font myFont -underline 0
...
bind . <Alt-m> {
    focus .bottom.module_combo
}
bind . <Alt-M> {
    focus .bottom.module_combo
}

# Bind Start: Alt+s / Alt+S (global)
bind . <Alt-s> {
    open_module_window
}
bind . <Alt-S> {
